...
const sncbHafas = createHafas(sncbProfile, 'pan-european-routing')
...
const normalizeStopName = (str) => {
	...
	str = slug(str.replace(/[\s-.]+/g, ' ')).toLowerCase()
...
const matchers = {
	...
	self: {
		...
		eurostar: (leg) => {
			...
			return /eur\s/i.test(line.line.name)
		},
	...
	db: {
		ice: (leg) => {
			if (!leg.line || !leg.line.name) return null
			return /ice\s/i.test(line.line.name)
		},
	...
	sncf: {
		tgv: (leg) => {
			if (!leg.line || !leg.line.name) return null
			return /tgv\s/i.test(line.line.name)
		},
