...
const endpoints = [
	...
	[0, vbbServiceArea, vbbEndpName, vbbClient, normalizeVbbStopName, normalizeVbbLineName],
	[0, hvvServiceArea, hvvEndpName, hvvClient, normalizeHvvStopName, normalizeHvvLineName],
	[0, insaServiceArea, insaEndpName, insaClient, normalizeInsaStopName, normalizeInsaLineName],
	[1, pkpServiceArea, pkpEndpName, pkpClient, normalizePkpStopName, normalizePkpLineName],
	[1, dbServiceArea, dbEndpName, dbClient, normalizeDbStopName, normalizeDbLineName],
	// SNCB has very sparse data
	[2, sncbServiceArea, sncbEndpName, sncbClient, normalizeSncbStopName, normalizeSncbLineName],
].sort((a, b) => a[0] - b[0])
...
const enrichLegFns = endpointPairs.map(([source, target]) => {
	...
	const matchLeg = async (srcLeg) => {
		debug(sourceEndpName, '->', endpName, 'matchLeg', srcLeg)
