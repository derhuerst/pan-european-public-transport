...
const enrichLegFns = endpointPairs.map(([source, target]) => {
	...
	const shouldEnrich = (srcLeg) => {
		...
		// const lName = normalizeSrcLineName(srcLeg.line.name || '')
		// const origName = normalizeSrcStopName(srcLeg.origin.name || '')
		// const destName = normalizeSrcStopName(srcLeg.destination.name || '')
